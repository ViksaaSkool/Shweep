...
- 🎨 **Sheep Color Choice**: White and black sheep are free; rainbow (Colorful) sheep unlock with an optional one-time purchase. Black sheep also switches the Start screen to the dark night background, and a first-launch, non-dismissable dialog makes the initial choice.
- ⚙️ **Settings**: Sheep color, Upgrades (the two optional one-time purchases plus Restore purchases), an About card (installed version, Privacy Policy, Terms of Service, contact support), and Invite friends. Changes apply only when you tap Save.
...
- 💾 **Local Persistence**: Sessions, the active-session checkpoint, preferences, and the free-sheep allowance are stored with multiplatform DataStore and are not sent to a Shweep-operated backend. RevenueCat is contacted only for products, purchases, restores, and entitlements.
- 🐏 **Sheep Allowance & Unlimited Sheep**: From version 2.0.0, count up to 35 free sheep; the 24-hour countdown starts when you count the 35th, and the allowance then resets. An optional one-time in-app purchase ("Unlimited Sheep") removes the allowance while the purchase entitlement is valid. The allowance is calculated and stored locally. Purchase and entitlement status is managed through RevenueCat using an anonymous installation identifier and can be restored with the same store account; a one-time notice explains the change when users update.
